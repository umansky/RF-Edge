...
#-import Vorpal data
xgrid,ygrid,fpare,fpari = ImportVorpal(vfile="VorpalDataOnUedgeGrid_NEW.h5")

##-show raw data from Vorpal

fig=plt.figure()
levs=np.linspace(-10.0,10.0,num=101)
plt.contourf(xgrid,ygrid,fpare,levs)
fig.suptitle('Vorpal F_par,e')
plt.xlabel('x [m]')
plt.ylabel('y [m]')
plt.colorbar()
plt.show()
...
fig=plt.figure()
levs=np.linspace(-1.0,1.0,num=101)
plt.contourf(xgrid,ygrid,fpari,levs)
fig.suptitle('Vorpal F_par,i')
plt.xlabel('x [m]')
plt.ylabel('y [m]')
plt.colorbar()
plt.show()
...
##-save it for later for scaling etc.
pondomfpari_use=np.nan_to_num(fpari) 
pondomfpare_use=np.nan_to_num(fpare)

#-plot as UEDGE objects
mp.plotvar(pondomfpari_use, title="Ion Fpar from Vorpal", iso=False)
mp.plotvar(pondomfpare_use, title="Electron Fpar from Vorpal", iso=False)

bbb.pondomfpari_use[:,:,0]=-pondomfpari_use
bbb.pondomfpare_use[:,:]=pondomfpare_use
