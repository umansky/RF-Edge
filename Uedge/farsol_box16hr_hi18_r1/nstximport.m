...
fname='mod3.009.h5' % Maxim's high-edge-density, nonuniform-grid case
...
disp('Are these the input file values?');
  numCells = [484 1632  12]
  lengths = [0.605 2.04 0.03]
...
for q=1:ind-1
 Nigrid(q,1+3:nrg-3)=Nigrid(ind,1+3:nrg-3);
end
...
plot3([-.05 .3 .3 -.05 -.05],[-1 -1 1 1 -1],1e20*[1 1 1 1 1],'r-')
set(gca,'ZScale','log')
view(0,0)
shading interp
% plot the UEDGE data
